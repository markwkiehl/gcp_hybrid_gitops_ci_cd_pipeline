...
echo Make sure to edit the variable "GCP_PROJ_ID" in the file gcp_constants.txt if you wish to build the project again.
echo WARNING: You cannot use the same bucket name of "%GCP_GS_BUCKET%" again later.  
echo Revise the variable "GCP_GS_BUCKET" in gcp_constants.txt
echo.
...
rem Delete terraform files terraform.tfstate & terraform.tfstate.backup
if EXIST "%PROJ_DIR%\terraform.tfstate" (
  CALL del "%PROJ_DIR%\terraform.tfstate" /Q
) 
rem Delete terraform files terraform.tfstate
if EXIST "%PROJ_DIR%\terraform.tfstate.backup" (
  CALL del "%PROJ_DIR%\terraform.tfstate.backup" /Q
) 
